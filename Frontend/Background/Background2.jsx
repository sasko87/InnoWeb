...
export default function Background2({
  ...
  useEffect(() => {
    ...
    function handleScroll() {
      ...
      const totalSections = nebulaCanvasesRef.current.length + 1;

      nebulaCanvasesRef.current.forEach((_, i) => {
        const start = sectionHeight;
        const end = sectionHeight * totalSections;
        const progress = (scrollTop - start) / (end - start);
        const opacity = Math.min(Math.max(progress, 0), 1);
        targetOpacitiesRef.current[i] = opacity;
      });
    }
